...
class Branch
{
    public:
            ...
            vector<pair<string, int>>hallsInBranch;
    ...
    for (int i = 0; i < numOfBranches; i++)
    {
        ...
        for (int j = 0; j < college[i].numOfBuilds; j++)
        {
            ...
            for (int k = 0; k < thisBuild.numOfFloors; k++)
            {
                ...
                {
                    college[i].hallsInBranch.push_back({thisFloor.hallsInFloor[z].name, thisFloor.hallsInFloor[z].volume});
...
void initialize()
{
    ...
    mem.clear();
    mem.resize(sz1, vector<int>(sz2, -1));
}
...
void buildDp(int idx, int msk)
{
    if (idx == N)
    {
        for (pair<int, int> s : toPrint)
            cout << h[s.first].first << ' ' << (s.second == -1 ? "free" :  group[g[s.second]].name) << '\n';
        cout << "\n\n-------------------------------------------------\n\n";
        return;
    }
...
void solve(Branch branch)
{
    ...
    vector<pair<string, int>>halls = branch.hallsInBranch;
